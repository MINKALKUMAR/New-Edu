...
export default function PastGlimpses() {
  ...
  // ---------- Speed (increased again by extra 20%) ----------
  const speeds = {
    s1: 45 * 0.2, // was 45*0.8 → now even faster
    s2: 57 * 0.2,
    s3: 51 * 0.2,
    s4: 63 * 0.2,
  };
  ...
  const styles = {
    ...
    // Horizontal lane animation
    strip: (duration, reverse) => ({
      display: "flex",
      whiteSpace: "nowrap",
      animation: `${reverse ? "scrollLeftReverse" : "scrollLeft"} ${duration}s linear infinite`,
    }),
    ...
    styleTag: `
      ...
      /* Responsive */
      @media (max-width: 768px) {
        .pg-item { width: 260px !important; height: 180px !important; }
      }
      @media (max-width: 520px) {
        .pg-item { width: 210px !important; height: 150px !important; }
      }
    `,

    // LEFT & RIGHT SHADOWS (gradient fade)
    shadowLeft: {
      position: "absolute",
      left: 0,
      top: 0,
      bottom: 0,
      width: "120px",
      background: "linear-gradient(to right, #0b1f3d, transparent)",
      zIndex: 5,
      pointerEvents: "none",
    },
    shadowRight: {
      ...
      bottom: 0,
      width: "120px",
      background: "linear-gradient(to left, #0b1f3d, transparent)",
      zIndex: 5,
      pointerEvents: "none",
    },
  };
  ...
  return (
    <section style={styles.section}>
      ...
      <div style={styles.stripContainer}>
        <Strip duration={speeds.s1} reverse={false} offset={0} />
        <Strip duration={speeds.s2} reverse={true} offset={4} />
        <Strip duration={speeds.s3} reverse={false} offset={8} />
        <Strip duration={speeds.s4} reverse={true} offset={12} />
      </div>
    </section>
  );
}
